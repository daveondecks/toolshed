...
# Function to generate tool images
...
def draw_tools_box(title, tools, color):
    ...
    # Draw the toolbox
    ax.add_patch(plt.Rectangle((0.1, 0.7), 0.8, 0.2, color='black', ec='black', lw=2))
    ax.text(0.5, 0.8, "Tools", ha='center', va='center', fontsize=12, color='white', fontweight='bold')
    
    # Draw tool slots
    for i, tool in enumerate(tools):
        ax.add_patch(plt.Rectangle((0.1, 0.5 - i * 0.15), 0.8, 0.12, color=color, ec='black', lw=2))
        ax.text(0.5, 0.56 - i * 0.15, tool, ha='center', va='center', fontsize=10, color='black')
    
    ax.set_title(title, fontsize=14, fontweight='bold')
    return fig

# Display toolboxes with selections
